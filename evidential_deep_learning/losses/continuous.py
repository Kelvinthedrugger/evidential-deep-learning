...
def Gaussian_NLL(y, mu, sigma, reduce=True):
    ...
    neglogprob = tf.math.log(sigma) + 0.5 * \
        tf.math.log(2*np.pi) + 0.5*((y-mu)/sigma)**2
    loss = tf.reduce_mean(neglogprob, axis=ax)
...
def Gaussian_NLL_logvar(y, mu, logvar, reduce=True):
    ...
    log_liklihood = 0.5 * (
        ...
        tf.math.log(2 * tf.constant(np.pi, dtype=logvar.dtype)) - logvar
    )
